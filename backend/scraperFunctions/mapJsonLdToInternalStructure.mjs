...
function mapJsonLdToInternalStructure(jsonData) {
    ...
    const mapNutrition = (nutritionData) => {
        ...
        return {
            ...
            servingSize: extractNutritionValue('servingSize'),
        };
    };
    ...
    function mapInstructions(instructions) {
        ...
        // Helper function to decode HTML entities in instruction text
        ...
        if (instructions.length > 0 && instructions[0]['@type'] === 'HowToStep') {
            ...
            return [{
                ...
                items: instructions.map((step, index) => ({
                    stepNumber: index + 1,
                    instruction: decodeHtmlEntities(step.text || "Instruction detail unavailable")
                })),
            ...
            if (section['@type'] === 'HowToSection') {
                    ...
                    items: section.itemListElement.map((step, stepIndex) => {
                        if (step['@type'] === 'HowToStep') {
                            return {
                                ...
                                instruction: decodeHtmlEntities(step.text)
                            };
                        }
                ...
                return {
                    ...
                    items: [{
                        ...
                        instruction: decodeHtmlEntities(section.text)
                    }],
                    sectionOrder: index + 1
                };
            }
    ...
    function mapIngredients(ingredients) {
        ...
        // Helper function to decode HTML entities in ingredient text
        const decodeHtmlEntities = (text) => he.decode(text);

        return ingredients.map(ingredient => decodeHtmlEntities(ingredient));
    }

    const mappedData = {
        recipeName: recipeData.name || 'No title provided',
        recipeDescription: he.decode(recipeData.description || 'No description provided'),
        imageUrl: getImageUrl(recipeData.image),
        ...
        ingredients: mapIngredients(recipeData.recipeIngredient || []),
