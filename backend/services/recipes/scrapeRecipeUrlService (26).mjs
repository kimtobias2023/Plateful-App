...
async function extractStructuredData(htmlContent) {
    ...
    let extractedRecipeData = null; // Declare a variable to hold the found recipe data

    $('script[type="application/ld+json"]').each((index, element) => {
        if (extractedRecipeData) return false; // If data has already been found, exit the loop
        try {
            const jsonLdData = JSON.parse($(element).html());
            console.log("Extracted JSON-LD Data:", jsonLdData);

            if (jsonLdData['@type'] === 'Recipe') {
                extractedRecipeData = mapJsonLdToInternalStructure(jsonLdData);
            } else if (jsonLdData['@graph']) {
                const recipeData = jsonLdData['@graph'].find(item => item['@type'] === 'Recipe');
                if (recipeData) {
                    extractedRecipeData = mapJsonLdToInternalStructure(recipeData);
                }
            }
        } catch (e) {
    ...
    // Return the extracted recipe data or null if no recipe data was found
