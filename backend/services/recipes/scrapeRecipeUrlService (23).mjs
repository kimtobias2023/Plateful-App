...
import recipeSitesConfig from '../../recipeScraper/recipeSitesConfig.mjs';
    ...
    for (let field of fields) {
        ...
        if (selector) {
            try {
                ...
                recipeData[field] = 'N/A'; // Use 'N/A', an empty string, or any other placeholder for missing or unspecified fields
            }
        } else {
...
const extractRecipeNotes = async (page, config) => {
    try {
        ...
        // Check if the recipe notes selector exists on the page
        const notesElements = await page.$$(recipeNotes);
        if (!notesElements || notesElements.length === 0) {
