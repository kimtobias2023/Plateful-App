...
function normalizeRecipeData(scrapedData) {
    ...
    let ingredientSections = normalizeIngredientSectionsBasedOnHeaders(scrapedData.ingredients);

    if (!ingredientSections.length || ingredientSections.some(section => !section.ingredientHeader)) {
        // Use headers and counts extracted from HTML content or pre-defined structure
        if (scrapedData.ingredientHeadersAndCounts) {
            ingredientSections = normalizeIngredientSectionsWithCounts(scrapedData.ingredients, scrapedData.ingredientHeadersAndCounts);
        } else {
            // No headers found, fallback to a single default section
            ingredientSections = [{
                ingredientHeader: "",
                ingredients: scrapedData.ingredients.map(ingredientLine => parseIngredient(ingredientLine)),
                sectionOrder: 1
            }];
        }
    }
...
// Function to normalize ingredient sections using headers and counts
function normalizeIngredientSectionsWithCounts(ingredientData, ingredientHeadersAndCounts) {
    let ingredientSections = [];
    let ingredientIndex = 0; // To keep track of which ingredient we're on

    // Loop through each header and its associated count of ingredients
    ingredientHeadersAndCounts.forEach(header => {
        // Slice the ingredients array to get the portion for the current header based on the count
        let sectionIngredients = ingredientData.slice(ingredientIndex, ingredientIndex + header.count);
        // Map each ingredient string to a parsed ingredient object
        let parsedIngredients = sectionIngredients.map(ingredientLine => parseIngredient(ingredientLine));

        // Add a new section with the header, parsed ingredients, and a section order
        ingredientSections.push({
            ingredientHeader: header.header,
            ingredients: parsedIngredients,
            sectionOrder: ingredientSections.length + 1
        });

        // Move the index forward by the count of ingredients for this header
        ingredientIndex += header.count;
    });

    return ingredientSections;
}

function normalizeIngredientSectionsBasedOnHeaders(ingredientData) {
    let ingredientSections = [];
