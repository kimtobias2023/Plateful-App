...
function normalizeRecipeData(scrapedData, ingredientHeaders, instructionHeaders, recipeNotes) {
    ...
    if (!scrapedData.ingredientHeaders || scrapedData.ingredientHeaders.length === 0) {
        normalizedData.ingredients = scrapedData.ingredients;
    } else {
        normalizedData.ingredientSections = normalizeIngredientSections(scrapedData.ingredients, scrapedData.ingredientHeaders);
    }

    // Handling flat structure for instructions
    if (!scrapedData.instructionHeaders || scrapedData.instructionHeaders.length === 0) {
        normalizedData.instructions = scrapedData.instructions;
    } else {
